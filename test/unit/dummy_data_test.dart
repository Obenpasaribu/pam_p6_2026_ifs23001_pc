...
void main() {
  group('DummyData.getPlantsData()', () {
    ...
    test('semua tanaman memiliki harga dengan format Rp', () {
      for (final plant in plants) {
        expect(plant.harga, startsWith('Rp'),
            reason: 'Harga harus dimulai dengan Rp untuk: ${plant.nama}');
      }
    });

    test('semua tanaman memiliki kegunaan yang tidak kosong', () {
      for (final plant in plants) {
        expect(plant.kegunaan, isNotEmpty,
            reason: 'Kegunaan kosong untuk: ${plant.nama}');
      }
    });
